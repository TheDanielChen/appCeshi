...
export default class IncrementSubmit extends Component{
    ...
    componentWillMount(){
        ...
        Util.fetch("area/jsArea",{}, (result) => {
            ...
            let other = {
                value:'0',
                label:'其他',
                children:[{label:'其他',value:'0',children:[{label:'其他',value:'0',children:null}]}]
            };
            data.push(result);
            data.push(other);
            console.log(result)
        ...
        AsyncStorage.getItem(STORAGE_KEY_EXPRESS,(error, ExpressData) =>{
            if (!error){
                if (ExpressData !== '' && ExpressData !== null){
                    ...
                    AsyncStorage.getItem(STORAGE_KEY_REALNAMELIST,(error, ExpressRealData) =>{
                        if (!error){
                            if (ExpressRealData !== '' && ExpressRealData !== null){
                                let data = JSON.parse(ExpressRealData);
    ...
    //关联保单
    selectBaodan(expressId,orderFlag){
        ...
        if(orderFlag=='1'){
            Util.fetch("mobile/express/realNameList", {expressId: expressId}, (result) => {
                console.log(result);
                if(result.code == "200" && result.data){
                    let items = result.data;
                    let newData =items[0];
    ...
    selectProvince(areaL1Id,areaL2Id,areaL3Id){
        ...
        for(let item of district){
            if(item.value==areaL1Id){
                province=item.label=='其他'?'':item.label;
                for(let ii of item.children){
                    if(ii.value==areaL2Id){
                        city=ii.label=='其他'?'':ii.label;
                        for(let j of ii.children){
                            if(j.value==areaL3Id){
                                area=j.label=='其他'?'':j.label;
                            }
                        }
                    }
                }
    ...
    zengzhiSubmit= async ()=>{
        ...
        if(expressId==''||expressId==null){
            ...
            this.timer = setTimeout(async()=>{
                await this.setState({isDisable:false})//2.5秒后可点击
            },2500);
            return false;
        }else{
            let areaL1Id;
            ...
            accidentInfo[0].remark = this.state.remark;
                        ...
                        <ScrollView style={{paddingRight:15,paddingLeft:15}}>
                            {(()=>{
                                return this.state.baodanData.map(item=>{
                                    return(
                                        <TouchableOpacity key={item.expressId} onPress={()=>{this.selectBaodan(item.expressId,item.orderFlag)}}>
                                        ...
                                        return(
                                            <View key={item.idCard}>
                                                ...
                                                <View style={styles.row}>
                                                    ...
                                                    <View style={{justifyContent:'center',height:46}}>
                                                        <Text style={{fontSize:14,color:'#6c6d6e',}}>{this.state.remark==''?'暂无':this.state.remark}</Text>
                                                    </View>
                                                </View>
